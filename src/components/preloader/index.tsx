...
// Minimum time the greeting splash stays on screen, even if the 3D scene
// finishes loading earlier — otherwise the greetings flash by unread.
const MIN_SPLASH_TIME = 2800;
function Preloader({ children, disabled = false }: PreloaderProps) {
  ...
  const bypassLoading = () => {
    ...
    if (elapsed >= MIN_SPLASH_TIME) return finish();
    if (bypassTimer.current) return;
    bypassTimer.current = setTimeout(finish, MIN_SPLASH_TIME - elapsed);
  };
